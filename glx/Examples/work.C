...
void
mesg(string message, int value)
{  
  MESGVAR(message,value);
}

void wakeup(glx* env, string message)
{
  mesg(message + string(": wakeup"));
  env->wakeup();
}
...
produce(void * arg)
{
  ...
  while( ! isDone() ){
    ...
    mesg("\t\tproducer: starting work...");
    glXWaitX(); 
    int N=10;
    int i=0;
    while( i<10 && ! isDone() && ! isHalted() ) {

      mesg("\t\tproducer: lock(&readyLock)...");
      pthread_mutex_lock(&readyLock);
      mesg("\t\tproducer: isReady",isReady);
      while( isReady==1) {
	int res = pthread_cond_wait(&readyCond,&readyLock);
	if( res != 0 ){
	  cerr << "pthread_cond_wait() failed ["<<res<<"]!"<<endl;
	  return 0;
	}
      ...
      GLenum err;
      if( itsObjID==MAXINT) {
	mesg("\t\tproducer: glGenLists");
	itsObjID = glGenLists(1);
	mesg("\t\tproducer: itsObjID",itsObjID);
	err = glGetError();
	assert(err==GL_NO_ERROR);
      }
      mesg("\t\tproducer: glNewList");
      glNewList(itsObjID, GL_COMPILE);
      err = glGetError();
      assert(err==GL_NO_ERROR);
      mesg("\t\tproducer: err",err);
	...
	glEnd();
      ...
      mesg("\t\tproducer: glEndList");
      glEndList();

      ++i;
      isReady=1;

      mesg("\t\tproducer: unlock(&readyLock)...");
...
void draw(glx* env, void*)
{
  bool consumed=false;
  mesg("=== draw ===");
  env->showMessage("'w' = start work");
  env->showMessage("'h' = halt work");
  env->showMessage("'d' = start work and dump out images");
  env->showMessage("'q' = quit");
  if( itsObjID==MAXINT ){
    mesg("consumer: done with draw");
    return;
  }
  glDisable(GL_LIGHTING);
  mesg("consumer: lock(&readyLock)...");
  pthread_mutex_lock(&readyLock);
  mesg("consumer: isReady",isReady);
  if( isReady == 1 ){
    consumed=true;
    glCallList(itsObjID);
    mesg("=== consuming ===");
  ...
  mesg("consumer: unlock(&readyLock)...");
  pthread_mutex_unlock(&readyLock);
  if( consumed )
...
main(int argc, char** argv)
{
  ...
  glx* g = new glx(app,display,init);
  g->showAxis(true);
  g->setPosition(10,10);
